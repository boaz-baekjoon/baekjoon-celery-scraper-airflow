...
with DAG(
    ...
    crawler = athena_s3.GlueTriggerCrawlerOperator(
        ...
        crawler_name= 'baekjoon_user_crawler'
    )
    
    run_query = athena_s3.XComEnabledAWSAthenaOperator(
        task_id='run_query',
        query=athena_query,
        output_location='s3://airflow-bucket-hajun/athena_user/',
        database='test',
        aws_conn_id='hajun_aws_conn_id',
    )
    ...
    move_results = athena_s3.S3FileRenameOperator(
        ...
        source_key='athena_user/{{ task_instance.xcom_pull(task_ids="run_query") }}.csv',
        destination_bucket='airflow-bucket-hajun',
        destination_key='athena_user/users.csv',
        aws_conn_id='hajun_aws_conn_id',
        dag=dag,
    )
